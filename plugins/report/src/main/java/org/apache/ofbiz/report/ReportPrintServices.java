...
public class ReportPrintServices {
    private static final String MODULE = ReportPrintServices.class.getName();
    private static final String RESOURCE = "ReportUiLabels";

    public static Map<String, Object> printReportPdf(DispatchContext dctx, Map<String, ? extends Object> context) {
        Locale locale = (Locale) context.get("locale");
        String screenLocation = "component://report/widget/report/ReportReportScreens.xml";
        String reportScreenName = "ReportReport";
        ...
        try {
            ...
            String errMsg = UtilProperties.getMessage(RESOURCE, "ReportGeneralErrorRenderingScreen", UtilMisc.toMap("errorString", e.toString()),
                    locale);
            Debug.logError(e, errMsg, MODULE);
            return ServiceUtil.returnError(errMsg);
        } catch (IOException e) {
            String errMsg = UtilProperties.getMessage(RESOURCE, "ReportIOErrorRenderingScreen", UtilMisc.toMap("errorString", e.toString()), locale);
            Debug.logError(e, errMsg, MODULE);
            return ServiceUtil.returnError(errMsg);
        }
        ...
        // put the reportId in the screen context, is a parameter coming into the service
        //reportScreenRenderer.getContext().put("reportId", context.get("reportId"));
        ...
        try {
            ...
            String errMsg = UtilProperties.getMessage(RESOURCE, "ReportSAXErrorRenderingScreen", UtilMisc.toMap("errorString", e.toString()),
                    locale);
            Debug.logError(e, errMsg, MODULE);
            return ServiceUtil.returnError(errMsg);
        } catch (ParserConfigurationException e) {
            String errMsg = UtilProperties.getMessage(RESOURCE, "ReportParserConfigurationErrorRenderingScreen", UtilMisc.toMap("errorString",
                    e.toString()), locale);
            Debug.logError(e, errMsg, MODULE);
            return ServiceUtil.returnError(errMsg);
        }
        ...
        */
        ...
        if (services.length > 0) {
            ...
            try {
                ...
                String errMsg = UtilProperties.getMessage(RESOURCE, "ReportUnableToPrintPDFFromXSL-FO", UtilMisc.toMap("errorString",
                        pe.toString()), locale);
