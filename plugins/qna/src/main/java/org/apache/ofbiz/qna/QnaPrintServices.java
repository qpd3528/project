...
public class QnaPrintServices {
    private static final String MODULE = QnaPrintServices.class.getName();
    private static final String RESOURCE = "QnaUiLabels";

    public static Map<String, Object> printReportPdf(DispatchContext dctx, Map<String, ? extends Object> context) {
        Locale locale = (Locale) context.get("locale");
        String screenLocation = "component://qna/widget/qna/QnaReportScreens.xml";
        String reportScreenName = "QnaReport";
        ...
        try {
            ...
            String errMsg = UtilProperties.getMessage(RESOURCE, "QnaGeneralErrorRenderingScreen", UtilMisc.toMap("errorString", e.toString()),
                    locale);
            Debug.logError(e, errMsg, MODULE);
            return ServiceUtil.returnError(errMsg);
        } catch (IOException e) {
            String errMsg = UtilProperties.getMessage(RESOURCE, "QnaIOErrorRenderingScreen", UtilMisc.toMap("errorString", e.toString()), locale);
            Debug.logError(e, errMsg, MODULE);
            return ServiceUtil.returnError(errMsg);
        }
        ...
        // put the qnaId in the screen context, is a parameter coming into the service
        //reportScreenRenderer.getContext().put("qnaId", context.get("qnaId"));
        ...
        try {
            ...
            String errMsg = UtilProperties.getMessage(RESOURCE, "QnaSAXErrorRenderingScreen", UtilMisc.toMap("errorString", e.toString()),
                    locale);
            Debug.logError(e, errMsg, MODULE);
            return ServiceUtil.returnError(errMsg);
        } catch (ParserConfigurationException e) {
            String errMsg = UtilProperties.getMessage(RESOURCE, "QnaParserConfigurationErrorRenderingScreen", UtilMisc.toMap("errorString",
                    e.toString()), locale);
            Debug.logError(e, errMsg, MODULE);
            return ServiceUtil.returnError(errMsg);
        }
        ...
        */
        ...
        if (services.length > 0) {
            ...
            try {
                ...
                String errMsg = UtilProperties.getMessage(RESOURCE, "QnaUnableToPrintPDFFromXSL-FO", UtilMisc.toMap("errorString",
                        pe.toString()), locale);
